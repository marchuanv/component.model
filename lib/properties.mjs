...
export class Properties {
    ...
    */
    clone(Class) {
        ...
        const { bag } = getPropertiesMap(this);
        const deserialised = {};
        for(const { properties } of classInfo) {
            const bagItems = bag.items.filter(i => properties.find(propName => propName === i.name));
            if (bagItems.length > 0) {
                const cloned = proto.constructor;
                const cloneExt = this.clone(proto);
                console.log();
            }
        }
        return deserialised;
