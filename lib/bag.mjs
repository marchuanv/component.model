...
export class Bag extends Relation {
    ...
    */
    on(name, onceOff = true, readOnly = true, isSerialised = false, callback) {
        const { items } = getBagMap(this);
        let bagItem = items.find(i => i.name === name);
        if (!bagItem) {
            bagItem = new BagItem(name, value, readOnly, isSerialised);
            items.push(bagItem);
        }
    ...
    */
     ...
     * @param { Boolean } isSerialised
    */
    set(name, value, readOnly = false, isSerialised = false) {
        const { items } = getBagMap(this);
        let bagItem = items.find(i => i.name === name);
        if (bagItem) {
            if (bagItem.readOnly) {
